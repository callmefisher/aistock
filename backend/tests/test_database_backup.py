...
class TestDatabaseExport:
    ...
    @pytest.mark.asyncio
    async def test_export_success(self, client: AsyncClient, auth_headers: dict):
    ...
    @pytest.mark.asyncio
    async def test_export_mysqldump_failure(self, client: AsyncClient, auth_headers: dict):
        with patch("api.database_backup.subprocess.run") as mock_run:
...
class TestDatabaseImport:
    ...
    @pytest.mark.asyncio
    async def test_import_rejects_non_sql(self, client: AsyncClient, auth_headers: dict):
    ...
    @pytest.mark.asyncio
    async def test_import_success(self, client: AsyncClient, auth_headers: dict):
        with patch("api.database_backup.subprocess.run") as mock_run:
    ...
    @pytest.mark.asyncio
    async def test_import_mysql_failure(self, client: AsyncClient, auth_headers: dict):
        with patch("api.database_backup.subprocess.run") as mock_run:

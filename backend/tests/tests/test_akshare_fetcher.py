...
class TestStockFetcher:
    ...
    def test_normalize_daily_bar_single_row(self):
        ...
        result = self.fetcher._normalize_daily_bar(df, '600000')
        assert len(result) == 1
        # 验证NaN值不被填充为0（移动平均线数据不足时应为NaN）
        assert pd.isna(result.iloc[0]['ma5'])
        assert pd.isna(result.iloc[0]['ma10'])
        assert pd.isna(result.iloc[0]['ma20'])
        assert pd.isna(result.iloc[0]['ma30'])
        assert pd.isna(result.iloc[0]['ma60'])
        assert pd.isna(result.iloc[0]['ma120'])
        assert pd.isna(result.iloc[0]['ma250'])
    ...
    def test_calculate_ma_default_periods(self):
        """测试均线计算默认周期(5/10/20/30/60/120/250)"""
        df = pd.DataFrame({
    ...
    def test_calculate_ma_extreme_volatility(self):
        """测试均线计算极端波动"""
        df = pd.DataFrame({
            'close': [10.0, 100.0, 10.0, 100.0, 10.0]
        })
        result = self.fetcher.calculate_ma(df, periods=[5])
        assert 'ma5' in result.columns
        assert result['ma5'].iloc[-1] == 46.0

    def test_calculate_ma_monotonic_increase(self):
        """测试均线计算单调递增"""
        df = pd.DataFrame({
            'close': [10.0, 11.0, 12.0, 13.0, 14.0, 15.0, 16.0, 17.0, 18.0, 19.0]
        })
        result = self.fetcher.calculate_ma(df, periods=[5])
        assert result['ma5'].iloc[-1] > result['ma5'].iloc[-2]

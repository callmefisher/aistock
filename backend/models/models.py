...
class StockPool(Base):
    ...
    workflow_id = Column(Integer, index=True, comment="生成工作流ID")
    date_str = Column(String(20), default="", comment="数据日期")
    file_path = Column(String(500), comment="Excel文件路径")
    total_stocks = Column(Integer, comment="股票总数")
    data = Column(JSON, comment="股票数据")
    filter_conditions = Column(JSON, comment="过滤条件")
    source_types = Column(JSON, comment="来源工作流类型列表")
    is_active = Column(Boolean, default=True, comment="是否启用")
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())

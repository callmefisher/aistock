...
END_DATE = "2026-04-29"
OUTPUT_FILE = "/Users/xiayanji/qbox/aistock/data/excel/7public站上20日线.xlsx"

MA20_DATE_COL = f"{START_DATE}至{END_DATE}站上20日线日期"
MA20_COUNT_COL = f"{START_DATE}至{END_DATE}站上20日线次数"
...
def main():
    ...
    code_ma20_dates = defaultdict(list)
    ...
    for rec in all_records:
        ...
        if not code:
            continue
        ma20_val = str(rec.get("站上20日线", "")).strip()
        if not ma20_val:
            continue
        code_records[code].append(rec)
        code_ma20_dates[code].append(rec["_data_date"])
    ...
    for code, recs in code_records.items():
        ...
        ma20_dates = sorted(set(code_ma20_dates.get(code, [])))
        base[MA20_DATE_COL] = ",".join(ma20_dates)
        base[MA20_COUNT_COL] = len(ma20_dates)
    ...
    ma20_count_col_idx = None
    for i, h in enumerate(headers):
        if h == MA20_COUNT_COL:
            ma20_count_col_idx = i + 1
            break
    ...
    if ma20_count_col_idx is not None:
        for r_idx in range(2, ws.max_row + 1):
            count_val = ws.cell(row=r_idx, column=ma20_count_col_idx).value
    ...
    print(f"总行数: {len(df)}")
    ma20_count_1 = df[df[MA20_COUNT_COL] == 1].shape[0]
    print(f"站上20日线次数=1 的证券数: {ma20_count_1}")

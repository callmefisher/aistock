...
describe('resolveTarget - 严格单数字前缀（多位数字不匹配）', () => {
  it('1adbd.xlsx 首位单数字 → 并购重组', () => {
    const r = resolveTarget('1adbd.xlsx')
    expect(r.status).toBe('resolved')
    expect(r.workflow_type).toBe('并购重组')
  })

  it('11xxx.xlsx 首位两位数字 → 未匹配', () => {
    const r = resolveTarget('11xxx.xlsx')
    expect(r.status).toBe('unresolved')
  })

  it('88xxx.xlsx 首位两位数字（即使 8 在 PREFIX 映射里） → 未匹配', () => {
    const r = resolveTarget('88xxx.xlsx')
    expect(r.status).toBe('unresolved')
  })

  it('23xxx.xlsx 首位两位数字 → 未匹配', () => {
    const r = resolveTarget('23xxx.xlsx')
    expect(r.status).toBe('unresolved')
  })

  it('11百日新高.xlsx 前缀两位数字但含子目录关键字 → 仍走子目录', () => {
    const r = resolveTarget('11百日新高.xlsx')
    expect(r.step_type).toBe('match_high_price')
    expect(r.sub_dir).toBe('百日新高')
  })

  it('8、板块涨跌幅排名 0422.xlsx → 涨幅排名（例外：单数字 8 + 含"涨跌幅"绕过板块）', () => {
  ...
  it('88板块涨跌幅xxx.xlsx 前缀非单数字 → 走板块子目录（例外不触发）', () => {
    const r = resolveTarget('88板块涨跌幅xxx.xlsx')
    expect(r.step_type).toBe('match_sector')
    expect(r.sub_dir).toBe('一级板块')
  })

  it('8板块数据.xlsx 含板块但不含涨跌幅 → 走板块子目录', () => {
    const r = resolveTarget('8板块数据.xlsx')
    expect(r.step_type).toBe('match_sector')
    expect(r.sub_dir).toBe('一级板块')
  })

  it('板块涨跌幅统计.xlsx 不以 8 开头 → 走板块子目录', () => {
